...
export class ApiRequests {
    ...
    static async verifyUrl(url: string) {
        return axios.get(
            `${apiPrefix}/sso/validateRedirect?redirectUrl=${url}`,
        );
    }

    static async verifyToken(token: string, type: number, name: string) {
        return axios.get(
            `${apiPrefix}/sso/validateToken/${token}?type=${type}&name=${encodeURIComponent(name)}`,
        );
    }
    ...
    static async getDisplay() {
        return axios.get(`${apiPrefix}/user/display`, {
    ...
    static async createSessionByToken(token: string, name: string) {
        return axios.post(
            `${apiPrefix}/sso/createSessionByToken/${token}`,
            { name },
            { withCredentials: true },
        );
    }
...
export class UsersEvents {
    ...
    static async createAccount(
        ...
        return axios.post(
            `${apiPrefix}/sso/createUser`,
            { userId, login, password, display_name },
            { withCredentials: true },
        );
    }
