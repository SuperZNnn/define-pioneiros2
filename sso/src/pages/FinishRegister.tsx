...
const FinishRegisterPage = () => {
    ...
    useEffect(() => {
        ...
        .then(res => {
            const resData = res.data as { message: string, token: { owner_id: number, token: string, token_type: number, expires_at: Date } }
    ...
    const onSubmit = (data: { display_name?: string, login: string, password: string, c_password: string }) => {
        if (userId){
            UsersEvents.createAccount(userId, data.login, data.password, data.display_name)
            .then(res=>{
                if (res.status === 201){
                    setUserId(undefined)
                    addToast({ message: 'Registrado com sucesso!', type: 'success', time: 3 })
            ...
            <FormStyles style={{ width: '45vh' }}>
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="input-group">
                        <input className="ipt-basic border" {...register('display_name')} placeholder="Insira seu nome de exibição" type="text"/>
                        {errors.display_name?<p className="error-message simple-warn">{errors.display_name.message}</p>:null}
                    </div>
                    <div className="input-group">
                        <input className="ipt-basic border" {...register('login')} placeholder="Crie seu Login" type="text"/>
                        {errors.login?<p className="error-message simple-warn">{errors.login.message}</p>:null}
                    </div>
                    <div className="input-group">
                        <input className="ipt-basic border" {...register('password')} placeholder="Crie sua Senha" type={`${viewPassword?'text':'password'}`}/>
                    ...
                    <div className="input-group">
                        <input className="ipt-basic border" {...register('c_password')} placeholder="Confirme sua Senha" type={`${viewPassword?'text':'password'}`}/>
                        {errors.c_password?<p className="error-message simple-warn">{errors.c_password.message}</p>:null}
                    </div>
                    <button className="btn" disabled={!userId} style={{ backgroundColor: 'var(--first-color)', color: '#fff' }} type="submit">Criar conta</button>
                </form>
            </FormStyles>
        </FinishRegisterStyle>
    )
