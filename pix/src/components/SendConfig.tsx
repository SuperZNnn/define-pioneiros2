...
const SendModal = ({close, usersList, userId, maxValue}: { maxValue: string, usersList: User[], userId: number, close: () => void }) => {
    ...
    const handleSubmit = () => {
        const max = Math.round(parseFloat(maxValue.replace(',', '.')) * 100) // converte para centavos
        ...
        // continue com envio normalmente
        setError("")
        console.log("Enviando", amount, "centavos")
        // ... envio aqui
    }
        ...
        <SendModalWrapper className={`${outAnim?'out':''}`}>
            ...
            <div className={`wrapper ${outAnim?'out':''}`}>
                <div className="input-group">
                    ...
                    {error && <p className="simple-text" style={{ color: '#c10000', textAlign: 'center', fontSize: '1rem' }}>{error}</p>}
                </div>

                <div className="input-group">
                    <h5 className="second-title">Para quem deseja enviar?</h5>
                    <select className="ipt-basic">
                        <option disabled selected className="second-title">Para quem deseja enviar?</option>
