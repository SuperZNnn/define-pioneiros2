...
const MemberCard = ({ user, userDisplay, border, modifyDisplay }: { modifyDisplay?: DisplayInfo, border?: boolean, user: User, userDisplay?: DisplayInfo }) => {
    ...
    useEffect(() => {
        ...
        .then(res=>{
            setDisplay(res.find(item => item.origin_id === user.id))
        })
    }, [modifyDisplay])
    ...
    return (
        <ViewCardStyle border={border?border:false}>
            ...
            <div className="image-container">
                {modifyDisplay?.photo?<img alt="Foto do perfil" src={`${modifyDisplay.photo}`}/>
                :<img alt="Foto do perfil" src={user.photo?`${user.photo.startsWith('blob')?user.photo:`data:image/jpeg;base64,${user.photo}`}`:'https://pioneirosdoadvento.com/assets/images/default_user.jpg'}/> } 
            </div>
            {modifyDisplay?.display_name?<h2>{modifyDisplay.display_name.toUpperCase()}</h2>:null}
            {!modifyDisplay?.display_name && display?.display_name?<h2>{display.display_name.toUpperCase()}</h2>:null}
            {!modifyDisplay?.display_name && !display?.display_name?<h2>{user.fullname}</h2>:null}
